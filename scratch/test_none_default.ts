...
async function getStockUsageWithNoneHandling(drinkId: number) {
  const usage: any[] = [];
  ...
  for (const slot of slots) {
    ...
    // Rule 1: Determine default option for the slot across ALL options (including None)
    ...
    for (const opt of validInventoryOptions) {
      ...
      // If the slot's default option is 'None', then this inventory option is NOT default!
...
async function main() {
  console.log("=== Testing None Default Handling for Iced Latte (ID 36) ===");
  const usage36 = await getStockUsageWithNoneHandling(36);
