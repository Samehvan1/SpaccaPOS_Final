...
import { 
  ...
  FileText, Coins, PackageCheck, AlertCircle, ShoppingBag, ArrowLeftRight 
} from "lucide-react";
...
export default function PurchasesAdmin() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [posList, supList, ingList, branchList, stockQuantities] = await Promise.all([
        api("/api/purchases"),
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        {hasPermission("purchases:manage") && (
          <div className="flex items-center gap-2">
            <Link to="/admin/purchases/transfers">
              <Button variant="outline" className="gap-2 font-bold uppercase tracking-wider border-primary/20 hover:bg-primary/5">
                <ArrowLeftRight className="h-4 w-4 text-primary" /> Stock Transfers
              </Button>
            </Link>
      ...
      <Dialog open={showTransfer} onOpenChange={setShowTransfer}>
        <DialogContent className="max-w-3xl border-primary/10 bg-card max-h-[85vh] flex flex-col overflow-hidden">
          <DialogHeader className="shrink-0 border-b border-primary/5 pb-4">
            <DialogTitle className="font-bold text-xl flex items-center gap-2">
              <ArrowLeftRight className="h-5 w-5 text-primary" /> Inter-Branch Stock Transfer
            </DialogTitle>
